...
右键托盘图标并打开“参数设置”。托盘菜单、字段名称、状态和自定义输入对话框均为中文。所有可调运行参数都在多级菜单中，菜单标题显示当前值，当前选项带勾选。选择新值后程序会取消当前事务、重建内部窗口管理器、立即保存并继续运行，不需要手动编辑配置文件，也不需要重启托盘程序。
...
拖动结束和前台窗口切换都会触发布局检查。启用居中开关后，非活动窗口通过普通点击、任务栏或键盘切换成为前台时会居中；如果该次激活同时进入拖动事务，则以用户拖动后的最终位置为准，不执行居中。若希望先观察计划或排查问题，从“参数设置 > 运行模式”选择“仅预览（DryRun）”；DryRun 会生成移动和重排计划，但不会调用 Win32 修改窗口。恢复实际管理时选择“应用窗口调整”。
