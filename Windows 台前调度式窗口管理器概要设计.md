...
1. 从同一快照中选择受管理、可见、当前桌面、非活动且非 topmost 的窗口。
2. 按 `zIndex` 降序枚举，即从最底层开始。
3. 每次仅模拟把一个候选提升到活动窗口正下方；活动窗口和其他窗口的相对顺序保持不变，所有受影响 `zIndex` 同步更新。
4. 在模拟后的顺序上重新运行完整位置求解；纯重排已经满足约束时允许位置计划为空。
5. 第一个完整合法方案立即返回；不得为了更短移动距离继续尝试较高层窗口。
6. 所有候选失败时返回无重排、无移动的 `Unsatisfiable`，不得输出部分计划。
