...
| MVP-A18 | 居中开关关闭后活动窗口保持原位，但被遮挡窗口仍正常修复 | `stage_manager_core`、`stage_manager_mvp_coordinator`、`stage_manager_tray` |
...
6. 从中文托盘依次修改运行模式、居中开关、边缘数、边缘长度、求解上限和失败阈值，确认当前项勾选且无需重启即可生效；对边缘长度和最大管理窗口数选择“自定义…”，分别验证合法输入会保存、越界或非整数输入会显示错误且不会关闭对话框；退出并再次启动后应保持用户明确设置的值。
...
9. 关闭“新激活窗口居中”，点击后方窗口使其成为活动窗口；确认该窗口保持原位，而原前台窗口被遮挡时仍会移动。重新开启后再切换到另一窗口，确认仅这次新的激活发生居中。
