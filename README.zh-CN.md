...
- 优先让每扇窗口露出"顶边 + 侧边"（两条独立可点击的边），并围绕活动窗口生成确定性的分段标题栏阶梯。后台窗口严格按现有 Z-order 排列：先尽量填充左上段；下一扇再放就会越出工作区时，剩余窗口改从右上段重新起排，并留出足够横向间隔以区分标题栏。仍有溢出时才依次使用左侧、右侧和下方，不会沿屏幕顶边横向接续。
