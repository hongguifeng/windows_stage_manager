...
5. 在没有合法布局时明确返回无解，不用越界或改变 Z-order 伪造成功。
...
- 管理器不主动改变尺寸、激活状态、owner/owned 关系和 Z-order。
...
5. 实现硬约束过滤：活动窗口不动、尺寸不变、工作区内、交互区达标、显示器/虚拟桌面合法、Z-order 不变。
...
2. 使用 `SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER`，不提升窗口 Z-order。
...
- 活动窗口保持用户位置，非活动窗口保持尺寸、激活和 Z-order。
