...
- 活动窗口位置由事务类型和配置确定：启用居中时，非活动到活动的前台切换使用工作区中心；关闭时保留原位；拖动始终使用用户最终位置。不合法布局返回 `Unsatisfiable`。
...
2. 实现所有受管理窗口的违规扫描；协调器按事务类型给出活动窗口的居中位置或用户拖动位置，纯位置求解器只把它作为固定约束参与计算。
3. 实现候选生成：当前位置、所有相关阻挡框边界、工作区边界、四方向和必要的水平/垂直组合位移。
4. 候选生成后去重，并明确记录来源和方向。
5. 实现硬约束过滤：活动窗口不被纯位置求解器再次移动、尺寸不变、工作区内、交互区达标、显示器/虚拟桌面合法；普通位置候选保持 Z-order。
6. 实现软代价：窗口中心到工作区中心的距离、移动窗口数量、总 Manhattan 距离、距稳定布局距离、边界偏离、边方向切换和平局键。
...
- 前台切换事务将新活动窗口居中，拖动事务保持用户最终位置；非活动窗口保持尺寸和激活状态，只有位置无解时允许按受控策略改变一个非活动窗口层级。
...
| T13 | Windows Snap/前台切换 | 新活动窗口居中，已活动窗口拖动后保持用户位置，最终状态可验证 |
...
任何版本发现持续自触发、活动窗口在非激活事务中被擅自移动、窗口越界或无法暂停，立即回滚到 DryRun/Disabled 行为，不等待后续算法优化。
...
2. 生成参数多级托盘菜单，在标题显示当前值并勾选当前选项；非预设旧值显示为只读“当前自定义值”。
...
交付门槛：全部参数均可由托盘访问，当前值可见，选择后无需重启即可生效；默认仍为 `dry_run=false`，发布包可开箱工作。

## 13. M12 中心导向布局

目标：避免固定边缘方向把窗口持续推向某个角，并让新活动窗口获得稳定、可预测的中心位置，同时保留用户对已活动窗口的直接控制。

1. 将修复候选的首要软代价改为窗口中心到工作区中心的 Manhattan 距离；原边缘组合分类仅保留作诊断。
2. 仅当 `FOREGROUND` HWND 与当前活动 HWND 不同时创建激活事务，并按视觉框与 placement 框的偏移计算当前显示器工作区居中位置。
3. 相同 HWND 的重复前台事件不创建事务；`MOVESIZESTART/END` 路径始终采用用户最终位置，同批前台切换加拖动也不居中。
4. 把激活居中计划与非活动窗口修复、Z-order fallback 一起求解和应用；完整方案无解或超过 `MaxMovesPerBatch` 时不应用部分计划。
5. 增加单窗口、其他显示器工作区、视觉框偏移、重复激活、激活拖动、居中加 peer 修复、Z-order fallback、DryRun 和移动预算自动化测试。
6. 在 `batch_complete` 日志中输出 `activation_centering_used`，同步需求、概要设计、用户指南、验收用例和 TODO。

交付门槛：Debug/Release 均完成 Configure、Build 和 27 项 CTest；启用时前台切换居中只发生一次，用户拖动位置不被覆盖，所有组合计划仍满足既有硬约束与后验验证。

## 14. M13 独立边缘、居中开关与中文参数菜单

目标：修正角部重复计数，让默认两边缘目标与视觉结果一致；同时让激活居中可关闭，并完成无需编辑配置文件的中文参数控制面。

1. 多边缘可见性扫描扣除相邻边缘共享的角部，补充单竖带失败、横纵双轴成功、完全覆盖必须双轴移动和链式最终快照测试；单边缘降级继续使用完整边段。
2. 增加 `CenterActivatedWindow` 配置、持久化和托盘开关；关闭时前台切换仍触发 peer 修复，但活动窗口不产生居中移动。
3. 将托盘入口、字段标题、选项、状态 tooltip、自定义对话框和错误提示全部中文化。
4. 为连续数值参数定义字段级闭区间和保留命令槽，增加“自定义…”对话框；合法值复用预设的依赖归一化、持久化和热重建路径。
5. 自动化测试覆盖纯模型范围/命令/依赖、Win32 菜单文字，以及真实进程打开对话框、键盘输入、确认、配置落盘、日志和进程存活。

交付门槛：Debug/Release 均完成 Configure、Build 和全量 CTest；默认两边缘不能由同一窄带的角部重复满足；居中开关即时生效；中文托盘可选预设并可输入合法自定义整数。

## 14. M15 完整布局、宽屏均衡与可视化设置

1. 全局求解超时后按 Z-order 从上到下逐窗修复，但每一步均保留完整阻挡关系，最终必须对全部受管理窗口重新验证；不得用单目标成功替代完整布局成功。
2. 中心距离分别除以工作区宽高，以短边为共同尺度生成整数代价；相同屏幕占比的横向和纵向偏移应等价。
3. 增加中文可视化设置对话框、DIP 到当前显示器像素换算、owner-draw 遮挡预览、恢复默认和一次性保存热重建。
4. 增加四窗口完整 fallback、宽屏候选排序、DPI 预览模型、对话框资源和真实进程操作测试。

交付门槛：每项独立提交；Debug/Release 均完成 Configure、Build 和全量 CTest；真实进程可以从托盘打开设置窗口、修改 DIP 参数、保存落盘并完成热重建。
