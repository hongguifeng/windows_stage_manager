...
- 活动窗口位置由事务类型确定：非活动到活动的前台切换使用工作区中心，拖动使用用户最终位置；不合法布局返回 `Unsatisfiable`。
...
交付门槛：Debug/Release 均完成 Configure、Build 和 27 项 CTest；前台切换居中只发生一次，用户拖动位置不被覆盖，所有组合计划仍满足既有硬约束与后验验证。
