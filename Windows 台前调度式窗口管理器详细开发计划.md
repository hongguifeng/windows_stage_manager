...
目标：用户无需编辑 `settings.ini` 或重启程序即可查看、选择并持久化全部运行参数。

1. 建立强类型参数字段、预设、命令编码/解码和依赖归一化模型；单元测试覆盖每个字段的全部命令、非法命令与关联约束。
2. 生成 `Settings` 多级托盘菜单，在标题显示当前值并勾选当前选项；非预设旧值显示为只读 custom current value。
3. 将参数命令接入生命周期：取消事务、停止并 join 协调器、应用并保存配置、更新 DryRun/健康阈值、使用新设置重建内部管理器。
4. 增加真实进程集成测试，在隔离配置目录中发送 DryRun 托盘命令，验证 true/false 两次落盘、`setting_changed` 日志、重建后存活和托盘退出。
5. Debug/Release 均执行 Configure、Build 和全部 CTest；真实桌面测试串行运行，测试前确认没有外部 `stage_manager.exe` 干扰。

交付门槛：15 类参数均可由托盘访问，当前值可见，选择后无需重启即可生效；默认仍为 `dry_run=false`，发布包可开箱工作。
