...
4. 任何无法满足硬约束的布局必须返回 `Unsatisfiable`，不得越界或让其他窗口跨越活动窗口；活动窗口只可在非活动到活动的切换且放置开关启用时靠下居中，拖动时必须保留用户位置；Z-order 只能作为纯位置明确无解后的受控 fallback。
...
- [x] M15-01 用完整布局增量修复替代单目标 fallback，确保所有受遮挡窗口通过后验验证。commit: a367761
- [x] M15-02 按工作区纵横比归一化中心距离，均衡横向与纵向留白。commit: 18919c8
- [x] M15-03 增加中文可视化设置窗口，解释参数并实时预览 DIP 效果。commit: c257d4a

## 第十一迭代：可辨识度算法整体替换

- [x] M16-01 四边使用独立动态长度和深度，顶部优先采用实际标题栏高度。commit: 15e7429
- [x] M16-02 新活动窗口改为水平居中、底部对齐，保留重复激活和用户拖动保护。commit: 45db15e
- [x] M16-03 显式生成左上/右上标题栏通道，并按通道负载确定性均衡。commit: f94b87d
- [x] M16-04 删除旧设置模型，增加可辨识度预设、四边独立参数和中文可视化配置。commit: 835d319
- [-] M16-05 用新算法完整替换需求、概要设计和开发计划，并增加文档回归约束。commit: 待本次验证提交

## 第十二迭代：RC15 发布

- [ ] M17-01 提升版本和发布资料到 `0.1.0-rc15`，完成 Debug/Release 全量验证并独立提交。
