...
参与者选择在协调器中先保留最上层后台窗口，再按直接遮挡及同类窗口优先筛选，最多 20 扇；这不改变真实 Z-order。部分结果还必须满足严格的 Z-order 前缀：第一个未达标窗口之后的更低窗口不能在同一批移动。候选截断与 beam 剪枝可能漏解，`Unsatisfiable` 仅表示本次有限搜索没有找到安全改善。
...
普通 `Unsatisfiable` 和 native 移动失败都不再触发自动熔断。协调器取消当前批次、保留补排意图并退避重试；只有用户主动停用或进程退出才会关闭管理。
...
### Native move failure isolation

高 DPI 一致性修复：TitleBarLeftHalf 的保护高度与阶梯步长统一使用 ceil(1.5 × titleBarHeight)，titleBarHeight 已是物理像素。该目标不再受旧 top_depth_dip 下限覆盖，否则 48 DIP 在 144 DPI 下会成为 72 像素，拒绝本来合法的 48 像素阶梯。其他曝光模式仍使用各自的设置。新增 144 DPI、旧深度 48 DIP、实际标题栏 32 像素回归，修复前失败，修复后要求两级目标为 y=452、404 且无违规。

当前最终规则：删除全部后备候选生成、屏幕边缘枚举和方向评分，只生成严格的 1.5 倍标题栏左上阶梯。目标越界报告 staircase_outside_work_area，目标在工作区内但曝光失败报告 staircase_blocked；两者均停止，不搜索其他坐标。此规则取代本文下方所有关于半标题栏后备搜索的历史说明。

阶梯失败原因分流：目标矩形仍在工作区内但曝光验证失败，标记 staircase_blocked 并停止，固定窗口标记 window_fixed。不得把此情况当作空间不足去枚举远处屏幕边缘。仅目标矩形超出工作区时进入原有空间不足后备分支。新增固定高层遮挡下连续重试不远跳的回归测试。

阶梯间距约束：每个后台窗口的可见左上角等于上一层确定后的可见左上角减去本层标题栏高度的 1.5 倍（两轴相同、向上取整）。旧位置不限制移动距离；已露出的后台窗口也参与同一阶梯。点击与重试使用相同规则，成功阶梯重复求解不漂移。只有阶梯候选无法满足工作区或曝光条件才进入半标题栏后备处理。取消之前错误的单次位移上限与已可见跳过逻辑。

2026-09-09 核心替换完成后的规则（取代下方历史实现记录）：受管窗口按真实 Z 序截取；标题栏求解器只保留一套逐层规划，删除旧 Quality/beam/递归前缀搜索。左上候选优先，保护上层已确定几何，不再保护未处理下层的旧位置。协调器不再删除后台 moves；失败不进入成功验证，不修改 managed 身份。执行前恢复 from 校验，实际目标与计划位置也必须一致。新增 accepted_prefix_count、first_unresolved_hwnd、solver_stop_reason 区分候选用尽、预算用尽和移动数量限制。旧日志中的 unsatisfiable 不应被解释为数学不可解证明。

验证新增下层旧位置不能否决上层、最终计划精确重放、受管数量上限下的 Z 序选择、失败后低层不执行且批次为 ApiError。实际桌面回放以及旧规则测试迁移仍需单独评估，简单回归通过不代表任意桌面全可解。

重试排队期间，普通 Reconcile 仅刷新快照，不得绕过 background_retry_at_ms 和静默期门槛再次求解；只有专用重试入口可到期执行。batch_complete 的 apply_attempted 区分是否调用执行器，未调用时 apply_status_code 为 not_run；成功刷新但没有求解的批次报告实际快照数量及 idle。

当前协调器使用逐层提交规则：每层先试标题栏 1.5 倍的左上偏移；失败后只对该层枚举半标题栏候选，按左上、非右移、其他方向的顺序选择，同时禁止遮住原本可见的受管标题栏。该层无候选则停止。协调器执行时开启 stopOnFailure，上层移动失败后停止下层计划，等待新快照重算。移除可能改变尺寸的 SetWindowPlacement 回退。

左上前缀与后缀搜索分开：后缀禁用再次启动左上阶梯，避免以活动窗口重新锚定。若后缀产生越过未满足窗口的移动，舍弃该后缀结果并保留左上前缀。输出移动计划先按 Z 序执行前缀，再执行合法后缀。

左上阶梯以活动窗口的可见左上角为起点，按 Z 序逐级排列。每一级向左、向上均偏移该后台窗口标题栏高度的 1.5 倍（向上取整到物理像素），后一级以前一级的新位置为锚点。换算 placement 坐标时保留不可见边框偏移；不再以工作区左上角为起点。超出工作区或标题栏可见性不满足时进入原有后缀处理。

2026-09-09：修正左上排列前缀被后缀超时取消的问题。后缀超时时保留已验证前缀、还原未接受的候选位置，并恢复前缀窗口的 managed 标志后重新扫描可见性。协调器以第一个未满足窗口的 Z 序作为下界，而非以最上层窗口作为下界。新增 prefix_regression 有限预算回归测试。

批量布局执行时，单个窗口被系统拒绝移动（例如 `ERROR_ACCESS_DENIED`）不会中断后续窗口。执行器记录失败窗口并继续应用其余计划；协调器在本轮验证中排除该窗口，避免失败窗口拖累整批重试。下一次新的用户激活或布局事务仍会重新尝试该窗口。
