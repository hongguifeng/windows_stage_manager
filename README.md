...
2. 程序常驻系统托盘，开箱默认：
   - `place_activated_window=true`：窗口由"未活动"变为"活动"时，自动放到"水平居中、竖直靠下"的位置（可在托盘改为其他 3×3 位置）；
...
- 窗口由"未活动"变为"活动"（鼠标点击、任务栏、Alt+Tab 等均可）时，把它的可视矩形放到你选择的位置：水平左/中/右 × 竖直上/中/下，默认**水平居中 + 竖直靠下**；窗口比工作区还高时保持标题栏贴住工作区顶部。
...
- 部分布局中，最近活动的窗口绝不为其他窗口让位，先保住它、再逐步改善更下方的窗口；实在没有安全方案时程序不做任何改动，并弹出一条"窗口布局暂时无解"通知（同一状态不会反复弹，重试有频率限制，窗口状态变化后自动重算）。

### 安全边界

- 不改窗口大小、不改 Z-order；每次批量移动都带位置/尺寸后校验。
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\stage_release.ps1 `
  ...
  -Version 0.1.0-rc17 -Commit (git rev-parse --short HEAD)
```
